...
export default function TokoRiwayatTransaksiPage() {
	...
	const load = async () => {
		...
		try {
			const [orderResult, invoiceResult] = await Promise.all([
				ordersService.listForToko({ page: 1, limit: 100 }),
	...
	return (
		<TokoStorefrontShell title="Riwayat Transaksi">
			{error ? (
					...
					<table className="min-w-full divide-y divide-slate-200 text-sm">
						...
						<tbody className="divide-y divide-slate-100">
							{loading ? (
								...
								<tr><td colSpan={6} className="px-4 py-4 text-slate-600">Tidak ada invoice.</td></tr>
							) : (
