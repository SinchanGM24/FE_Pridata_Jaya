...
const getProductPreviewImage = (product: Product) =>
	product.catalogProduct?.imageList?.find(Boolean) ?? product.productDetail?.imageList?.find(Boolean) ?? "";
									...
									<tr key={product.id}>
										<td className="px-4 py-3">
											{previewImage ? (
												<Image
													src={previewImage}
													alt={product.catalogProduct?.marketingName ?? product.name}
													width={72}
													height={72}
													unoptimized
												...
												<div className="flex h-16 w-16 items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 text-[10px] font-medium uppercase tracking-[0.12em] text-slate-400">
													Belum ada
												</div>
											)}
										</td>
