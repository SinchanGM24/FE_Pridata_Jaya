				...
				<div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
					<div>
						...
						<p className="text-xs text-slate-600">
							Pantau tagihan toko kelolaan untuk follow-up pembayaran dari sisi sales.
						</p>
					</div>
					<Link
						href={`/sales/toko-kelolaan/${storeId}/riwayat-transaksi`}
			...
			<section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
				...
				<table className="min-w-full divide-y divide-slate-200 text-sm">
					...
					<tbody className="divide-y divide-slate-100">
						{loading ? (
							<tr><td colSpan={6} className="px-4 py-8 text-center text-slate-500">Memuat data tagihan...</td></tr>
						) : rows.length === 0 ? (
							<tr><td colSpan={6} className="px-4 py-8 text-center text-slate-500">Tidak ada tagihan berjalan untuk toko ini.</td></tr>
						) : (
							rows.map((item) => (
								<tr key={item.id}>
